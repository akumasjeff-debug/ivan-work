...
| `電玩庫存表/` | 電玩庫存表 | PChome 廠商商品庫存管理，**本體是 Google 試算表**（六欄：廠商名稱/商品編號/商品名稱/可賣量/成本/售價，缺貨標紅）。流程＝PChome 後台匯出檔（Downloads .xls 實為 xlsx）→ `匯入轉換.py` 轉六欄進剪貼簿 → 貼試算表 A1。`reports/` 含成本已 gitignore；試算表網址見本夾 `OVERVIEW.md` | `電玩庫存表/OVERVIEW.md` | `py 電玩庫存表\匯入轉換.py`（轉檔）；表建置＝Apps Script 跑 `setup.gs` |
