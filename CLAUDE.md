...
| `電玩庫存表/` | 電玩庫存表 | PChome 廠商商品庫存管理，**本體是 Google 試算表**：「庫存表」分頁＝最新快照七欄（廠商名稱/商品編號/商品名稱/可賣量/成本/售價/來源廠商，缺貨標紅）＋「可賣量歷史」分頁（一欄一匯入日期，週一比庫存差距）。流程＝PChome 後台匯出檔（Downloads .xls 實為 xlsx）→ `py 電玩庫存表\匯入轉換.py` → **經 GAS Web App 直接寫入試算表**。`reports/`（含成本）與 `config.local.json`（token+網址）已 gitignore；試算表網址見本夾 `OVERVIEW.md` | `電玩庫存表/OVERVIEW.md` | `py 電玩庫存表\匯入轉換.py`（一鍵更新）；表建置＝Apps Script 跑 `setup.gs` |
