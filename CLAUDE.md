...
| `電玩庫存表/` | 電玩庫存表 | 自家實體電玩商品進出貨庫存管理，**本體是 Google 試算表**（商品主檔／進貨／出貨／庫存總覽自動彙算＋低庫存標紅）；本資料夾只放一鍵建置腳本 `apps-script/setup.gs` 與文件，無敏感資料。試算表網址見本夾 `OVERVIEW.md`，公式與唯一鍵規則見 `HANDOVER.md` | `電玩庫存表/OVERVIEW.md` | 無（Google Sheet 雲端使用；建表＝新試算表跑 `setup.gs`） |
