...
| `pokemon-funsite/` | **四館旗艦館共用 repo**（寶可夢／PS／Xbox／任天堂） | **2026-08-10 起四個 PChome 品牌旗艦館集中在此**，一館一子夾：`pokemon/`、`ps/`、`xbox/`、`nintendo/`（含 `pikmin.htm` 皮克敏專頁、`game/game.html`）。**獨立私有 git repo**（github.com/akumasjeff-debug/pokemon-funsite），與同事協作維護（一次一人、pull→改→給 diff→push→打包 zip 上傳 PChome）。已在本 repo `.gitignore` 排除。**打包 zip 以各館子夾為根**（zip 內不含 `pokemon/` 這層）。機敏檔 `*/缺漏檢查/firebase帳密.txt` 與 `輸出/` 報表不進版控 | `pokemon-funsite/CLAUDE.md` | 無（靜態頁） |
...
| `PS旗艦館/` | PS旗艦館 | PChome PlayStation 品牌旗艦館（sites/playstation）：前台+本機後台+Firebase 資料庫+每日 GAS 自動化（備份/巡檢/PID切換）+每週一缺漏/缺貨例行。**實際檔案 2026-08-10 起在 `pokemon-funsite/ps/`（四館共用私有 repo）**，本資料夾僅登錄點，已在 `.gitignore` 排除。交接細節看 `pokemon-funsite/ps/HANDOVER.md` | `PS旗艦館/OVERVIEW.md` | 無（改完打增量 zip 上傳 PChome） |
| `Xbox旗艦館/` | Xbox旗艦館 | PChome Xbox 品牌旗艦館（sites/xbox）：前台（控制器/Game Pass/YouTube/Banner）+本機後台+Firebase（與 PS 同一專案）+每日 GAS（GA4 日報/賣場巡檢）+每週一缺漏/缺貨例行（與 PS 一起）。**實際檔案 2026-08-10 起在 `pokemon-funsite/xbox/`（四館共用私有 repo）**，本資料夾僅登錄點，已在 `.gitignore` 排除。交接細節看 `pokemon-funsite/xbox/HANDOVER.md` | `Xbox旗艦館/OVERVIEW.md` | 無（改完打增量 zip 上傳 PChome） |
...
| `任天堂旗艦館/` | 任天堂旗艦館 | PChome Nintendo 品牌旗艦館（sites/nintendo）改造案：**規劃階段（2026-07-14 評估完成，尚未動工）**。目標＝套 PS 館模式（Firebase+後台+GAS+例行）並保留現有角色分區/雙世代導覽優點；另做「翻面」非本家 Switch 遊戲面。**實際檔案 2026-08-10 起在 `pokemon-funsite/nintendo/`（四館共用私有 repo，含 `pikmin.htm` 皮克敏專頁、`game/game.html`）**，本資料夾僅登錄點，已在 `.gitignore` 排除。決策與商品源賣場代碼看本夾 `OVERVIEW.md`，交接細節看 `pokemon-funsite/nintendo/HANDOVER.md` | `任天堂旗艦館/OVERVIEW.md` | 無（改完打增量 zip 上傳 PChome，`nintendo_YYYYMMDD.zip`） |
...
| `廢文影片產生器/` | 廢文影片產生器 | 廢文文字→靜態圖＋字幕＋**星爺（周星馳）配音**的直式短片，純娛樂**不商用**。配音**共用 `ivan-video` 的 F5-TTS**（星爺原聲當參考音、零樣本克隆、不訓練）：**F5 只能家機跑**，公司機只能斷句／`--dry-audio` 排版預覽。字幕時間軸依配音實際時長自動對齊。輸出/wav/圖已 `.gitignore` | `廢文影片產生器/OVERVIEW.md` | `python make_video.py --lines inputs/lines.json --audio-dir output/voice --image inputs/bg.png --out output/廢文_HHMMSS.mp4`（bash） |
