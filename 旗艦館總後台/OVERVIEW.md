...
## 任天堂館 v9：子分頁＋版頭隱藏/上限＋三廠廣告工作台

- **子分頁**：任天堂館內容拆三頁（🏠 本家主頁／🌱 皮克敏專頁／🔄 非本家頁），sticky 分頁列在館 header 下。
- **版頭工作台抽成 `makeBench()` factory**，本家（NB→`nintendo_banners`）與非本家三廠廣告（TB→`nintendo_tp_banners`，
  PC 1644×604 單圖、檔名 `tpBanner-N.jpg`）兩實例共用。
- **卡片 👁 隱藏**：寫 `hidden` 欄位，前台 `bannerActive()` 過濾（index.html 與 game/game.html 都改了，要出 zip 上傳）。
- **前台輪播張數上限**：`nintendo_banner_settings`｛`max_show` 本家、`tp_max_show` 非本家（0＝預設 8）｝，
  區塊 header 的輸入欄隨「儲存順序與設定」一併寫入。
- **手機版圖單獨上傳**：卡片編輯面板「📱 手機版另外傳一張」→ 裁 768×698、該列轉回待上傳（比照 Xbox）；
  另外傳過的手機圖不會被裁切滑桿蓋掉。

## 任天堂館 v10：非本家頁遊戲牆工作台（視覺化）

- **🧱 遊戲牆工作台**：非本家子分頁新增卡片牆——與前台 `game.htm` 同一套即時抓取／去重邏輯
  （CANON／normTitle／isBundle 複製自前台，**改任一邊要同步另一邊**），首次切到該分頁自動載入。
- 每張卡直接操作：**⭐ 置頂**（依點選順序＝店長精選排序）、**🚫 隱藏**、**🎯 類型**（勾選面板覆蓋自動歸類）、
  **📅 補發售日**（點日期列改，存新節點 `nintendo_tp_dates`，優先於賣場 Nick 自動抓的日期）。
- 卡片牆分區＝前台同款預覽（精選→即將發售→最新發售→全部遊戲）；另有 ⭐／🚫／❗缺日期 過濾頁籤與品名搜尋。
- 改完按「⚡ 同步儲存」一次寫回三節點（`nintendo_tp_overrides`／`nintendo_tp_genres`／`nintendo_tp_dates`）；
  有未儲存變更時顯示紅點、伺服器回波不會蓋掉。舊 textarea 表單收進「🛠 進階備援」摺疊。
- 對應前台大改版（2026-09-03）：非本家頁搬到站台根 **`game.htm`**（舊 `game/game.html` 留轉址 stub）；
  商品牆改以**發售日**為主軸（三大區塊＋可切月份牆）；卡面顯示發售日／倒數／預購／🆕 角標。
...
- v9（2026-08-27）：任天堂館拆三個子分頁（本家／皮克敏／非本家）；版頭工作台抽成 makeBench() factory，三廠廣告也改卡片工作台；卡片 👁 隱藏＋前台輪播張數上限（nintendo_banner_settings）＋手機版圖單獨上傳。前台 index.html/game.html 同步支援 hidden 與上限（需上傳 zip）。
- v10（2026-09-03）：非本家頁新增「🧱 遊戲牆工作台」（卡片式 ⭐置頂／🚫隱藏／🎯類型／📅補發售日，寫新節點 nintendo_tp_dates；前台同款分區預覽＋缺日期頁籤）；舊 textarea 收進備援摺疊。配合前台大改版：game/game.html → 站台根 game.htm、商品牆改發售日主軸（三大區塊＋月份牆切換）。
