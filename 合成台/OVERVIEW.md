...
- **編輯工具列（畫布下方，單選一個圖片/框架圖層時出現）**：去背/裁切/旋轉/翻轉/置換都在這，圖層列只留排序、複製、鎖定、顯示、透明度、移除
  - 去背三式（結果記在 asset 上共用，可逐次堆疊）：✂AI（CDN 模型）、✂本機（四邊單色 flood-fill）、💧吸管（點背景色去除）；本機/吸管以原圖解析度輸出＋邊緣柔化抗鋸齒；↩原圖一鍵還原
