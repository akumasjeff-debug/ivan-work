...
- **編輯工具列（畫布下方）**：單選圖片/框架圖層＝全功能；多選＝旋轉/翻轉批次操作。圖層列只留排序、複製、鎖定、顯示、透明度、移除
  - 去背三式（結果記在 asset 上共用，可逐次堆疊）：✂AI（CDN 模型）、✂本機（四邊單色 flood-fill）、💧吸管（點背景色去除，強度滑桿就在旁邊）；本機/吸管以原圖解析度輸出＋邊緣柔化抗鋸齒；「↶退一步」回上一次去背（blob key `cutprev:<id>`，保留一層）、↩原圖全清
  - 裁切：8 控點調整＋拖圖身平移；裁切模式顯示半透明鬼影（看得到被切掉的部分）；「▣↺重設」一鍵還原整張圖；旋轉/翻轉下控點方向與位置有補正
- 畫布檢視縮放：Ctrl+滾輪 1~4 倍（以游標為中心），點下方「縮放 %」重設；只影響顯示不影響輸出
